...
| 模型、Playwright、按需 bridge、通用缓存、隔离区 | 仅由用户手动管理；Playwright 与可选 bridge 只在明确 plan → 确认 → apply 后下载到数据根 |
...
服务默认仅监听 `127.0.0.1`。本地配置页面不回显密钥，也不写访问日志；诊断仅返回脱敏的就绪状态和分类总量，不返回绝对路径、文件名、账号、Cookie、Profile、日志或任务内容。不要把 `.env`、`config/profile_registry.json`、浏览器资料、SQLite、Cookie、日志、报告、缓存或本机截图上传到 issue、仓库、归档或 Release。
