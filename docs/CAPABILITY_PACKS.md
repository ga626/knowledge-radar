...
| 模型服务 | 需要视频、图片或模型理解时 | 可选模型 API 配置 | 调用可能计费；本地转写模型与媒体缓存不在能力中心配置。 |
| 登录平台与招聘 | 实际访问需要登录的平台时 | 你的浏览器 Profile 与本地状态 | 验证码、风控或平台策略会要求你手动完成操作，或返回明确降级。 |

能力中心只展示上述需要填写 Key、URL 或完成人工登录的配置。诊断与隐私属于“设置与帮助”，不再伪装成一个待配置能力。

本地组件页按功能展示，而不是按“电脑已有软件 / 任务时下载”分类。插件本体只安装控制台与基础研究路径；额外组件都必须在本地控制台经过“生成计划 → 第二次确认安装”，不会随插件本体安装或打开页面而下载。

| 按需能力 | 下载到哪里 | 何时需要 | 不会做什么 |
| --- | --- | --- | --- |
| Playwright Chromium | 网页采集与自动化 | 动态网页或受支持的浏览器流程 | 不自动登录、不调用付费 API。 |
| 小红书诊断 bridge | 平台账号与浏览器协作 | 需要本地 bridge 诊断时 | 需要 Node.js/npm；不登录、不绕过验证码、不自动变成生产兜底；完成后需刷新或重启 Codex。 |
| 媒体下载器 | 本地媒体理解与转写 | 需要获取公开音视频时 | 仅在控制台确认后安装；实际任务仍遵守内容访问规则。 |
| 本地转写运行时与基础模型 | 本地媒体理解与转写 | 选择本地音视频转写时 | 运行时和模型权重分别确认下载；模型只保存到产品数据目录。 |

Chrome、Node.js/npm、FFmpeg 不再单独归入“电脑已有软件”：它们分别显示在平台账号或本地媒体处理功能内，提供官方安装引导和重新检测。Camoufox、FunASR、sherpa-onnx 均为实验/候选路径，默认关闭且没有普通安装入口。
