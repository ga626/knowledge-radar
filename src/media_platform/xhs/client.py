...
from runtime.redaction import redact_url
...
class XiaoHongShuClient(AbstractApiClient):
    def __init__(
        ...
        *,
        ...
        browser_context=None,  # Accept BrowserContext for native API requests
    ):
    ...
    async def _pre_headers(self, url: str, data=None) -> Dict:
        ...
        utils.logger.info("[_pre_headers] request signing prepared")
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        """
        通过page.evaluate()在浏览器内执行fetch，完全绕过反爬检测。
        """
        return_response = kwargs.pop("return_response", False)
        headers = kwargs.pop("headers", {})
        data = kwargs.pop("data", None)

        # 使用浏览器原生fetch，credentials: 'include'自动带cookie
        js_code = f"""
        (async () => {{
            try {{
        ...
        """
        result_str = await self.playwright_page.evaluate(js_code)
        result = json.loads(result_str)

        if result["status"] == 0:
            raise Exception(f"[request] Fetch failed: {result['body']}")

        status_code = result["status"]
        response_text = result["body"]

        if status_code == 471 or status_code == 461:
            raise Exception("[request] 出现验证码，请求失败")

        if return_response:
            return response_text
        resp_data: Dict = json.loads(response_text)
        if resp_data.get("success"):
            return resp_data.get("data", resp_data.get("success", {}))
        elif resp_data.get("code") == self.IP_ERROR_CODE:
            raise IPBlockError(self.IP_ERROR_STR)
        else:
            raise DataFetchError(resp_data.get("msg", None))
    ...
    async def get_note_media(self, url: str) -> Union[bytes, None]:
        async with httpx.AsyncClient() as client:
            ...
            if not response.reason_phrase == "OK":
                utils.logger.error(
                    f"[XiaoHongShuClient.get_note_media] request {redact_url(url)} failed with HTTP {response.status_code}"
                )
                return None
            else:
    ...
    async def update_cookies(self, browser_context: BrowserContext):
        ...
        self.cookie_dict = cookie_dict
        key_cookies = {name: name in cookie_dict for name in ("web_session", "a1", "webId")}
        utils.logger.info(f"[update_cookies] cookie refresh completed: total={len(cookie_dict)}, key_cookie_presence={key_cookies}")
    ...
    async def get_note_comments(
        ...
        """
        获取一级评论的API - 使用浏览器原生fetch，绕过httpx反爬检测
        """
        ...
        url = f"{self._host}/api/sns/web/v2/comment/page?{params}"
        ...
        try:
            ...
            """
            resp_text = await self.playwright_page.evaluate(js_code)
            data = json.loads(resp_text)
            if "error" in data and not "success" in data:
                raise Exception(f"Browser fetch error: {data['error']}")
            return data
        except Exception as e:
            utils.logger.warning(f"[get_note_comments] Browser fetch failed for {note_id}: {e}, falling back")
            return await self._legacy_get_note_comments(note_id, xsec_token, cursor)

    async def _legacy_get_note_comments(
        self, note_id: str, xsec_token: str, cursor: str = ""
    ) -> Dict:
        """Fallback to original httpx-based comment fetching"""
