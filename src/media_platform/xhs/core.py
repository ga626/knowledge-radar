...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        async with async_playwright() as playwright:
            ...
            # stealth.min.js prevents website from detecting the crawler.
            await self.browser_context.add_init_script(path="libs/stealth.min.js")
            self.context_page = await self.browser_context.new_page()

            # 先打开首页
            utils.logger.info("[XiaoHongShuCrawler] Navigating to homepage...")
            await self.context_page.goto(self.index_url, wait_until="domcontentloaded")
            await asyncio.sleep(2)

            # 登录流程（如果已登录，login_by_qrcode 会快速检测到并跳过）
    ...
    async def search(self) -> None:
        """
        使用Playwright浏览器原生导航进行搜索。
        让浏览器处理所有TLS指纹、Cookie、签名(webmsxyw)，通过page.expect_response()拦截API响应。
        """
        utils.logger.info("[XiaoHongShuCrawler.search] Begin search via browser navigation")
        xhs_limit_count = 20

        for keyword in config.KEYWORDS.split(","):
            source_keyword_var.set(keyword)
            utils.logger.info(f"[XiaoHongShuCrawler.search] Current keyword: {keyword}")

            collected_total = 0
            page_num = 1
            has_more = True

            while has_more and collected_total < config.CRAWLER_MAX_NOTES_COUNT:
                utils.logger.info(f"[XiaoHongShuCrawler.search] Fetching page {page_num} for '{keyword}'")

                if page_num == 1:
                    # 第一页：导航到搜索页面，浏览器自动处理签名
                    search_url = (
                        f"https://www.xiaohongshu.com/search_result?"
                        f"keyword={urllib.parse.quote(keyword)}&source=web_search_result_notes"
                    )
                    try:
                        async with self.context_page.expect_response(
                            lambda resp: (
                                "/api/sns/web/v1/search/notes" in resp.url
                                and resp.status == 200
                            )
                        ) as response_info:
                        ...
                        utils.logger.info(f"[search] Page {page_num} API response received")
                    except Exception as e:
                        utils.logger.error(f"[search] Page {page_num} failed: {e}")
                        break
                else:
                    # 后续页：滚动到底部触发无限滚动加载
                    try:
                        scroll_future = asyncio.get_event_loop().create_future()

                        def _page_handler(resp):
                            if (
                                "/api/sns/web/v1/search/notes" in resp.url
                                and resp.status == 200
                                and not scroll_future.done()
                            ):
                                scroll_future.set_result(resp)

                        self.context_page.on("response", _page_handler)
                        # 滚动到底部
                        await self.context_page.evaluate(
                            "window.scrollTo(0, document.body.scrollHeight)"
                        )
                        # 等待API响应
                        response = await asyncio.wait_for(scroll_future, timeout=15)
                        resp_json = await response.json()
                        utils.logger.info(f"[search] Page {page_num} via scroll, API response received")
                    except asyncio.TimeoutError:
                        utils.logger.info("[search] Scroll timeout - no more content")
                        break
                    except Exception as e:
                        utils.logger.error(f"[search] Scroll page {page_num} error: {e}")
                        break
                    finally:
                        self.context_page.remove_listener("response", _page_handler)

                # 解析 API 响应
                if not resp_json.get("success"):
                    utils.logger.error(f"[search] API returned not success: {resp_json.get('msg', 'unknown')}")
                    # 如果是"登录已过期"但仍然获取到了一些数据，继续处理
                    if not resp_json.get("data"):
                        break

                search_data = resp_json.get("data", {})
                items = search_data.get("items", [])
                has_more = search_data.get("has_more", False)

                if not items:
                    utils.logger.info(f"[search] No items on page {page_num}")
                    break

                utils.logger.info(f"[search] Page {page_num}: got {len(items)} items, has_more={has_more}")
