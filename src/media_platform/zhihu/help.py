...
try:
    _NODE_PATH = find_node_exe()
except FileNotFoundError:
    _NODE_PATH = None

ZHIHU_SGIN_JS = None
_ZHIHU_JS_SOURCE = None


def sign(url: str, cookies: str) -> Dict:
    """
    zhihu sign algorithm - 直接调用Node.js执行签名，避免execjs兼容问题
    """
    global ZHIHU_SGIN_JS, _ZHIHU_JS_SOURCE, _NODE_PATH

    if not _ZHIHU_JS_SOURCE:
        with open("libs/zhihu.js", mode="r", encoding="utf-8-sig") as f:
            _ZHIHU_JS_SOURCE = f.read()

    # 用subprocess直接调用node执行JS
    try:
        # 用stdin传参数，避免引号转义问题
        ...
        if r.returncode == 0:
            ...
            return {
                "x-zst-81": result.get("x-zst-81", ""),
                "x-zse-96": result.get("x-zse-96", ""),
            }
        else:
            raise Exception(f"Node sign failed: {r.stderr}")
...
class ZhihuExtractor:
    ...
    def _extract_content_or_comment_author(author: Dict) -> ZhihuCreator:
        ...
        try:
            ...
            log.warning("[ZhihuExtractor._extract_content_or_comment_author] User Maybe Blocked. %s", e)
        return res
